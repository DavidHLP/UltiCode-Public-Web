...
import { Trophy, Code2, Database, BrainCircuit, Share2, Building2, List, ArrowRight } from 'lucide-vue-next';
    ...
    <section class="space-y-6">
      ...
      >
        <CarouselContent class="-ml-4 pb-4">
          ...
          >
            <Card class="group relative overflow-hidden border-muted bg-card/50 hover:bg-card transition-all duration-300 hover:border-primary/50 h-full cursor-pointer">
                ...
                <div class="flex gap-3 relative z-10">
                  ...
                  <div class="flex flex-col flex-1 min-w-0 py-0.5">
                    <div class="flex items-start justify-between gap-2">
                      <h3 class="font-bold text-sm leading-tight tracking-tight group-hover:text-primary transition-colors truncate">
                        {{ item.title }}
                      </h3>
                    </div>
                    
                    <p class="text-[11px] text-muted-foreground line-clamp-2 mt-1 leading-relaxed">
                      {{ item.description }}
                    </p>

                    <div class="flex items-center justify-between mt-2.5">
                      <span class="inline-flex items-center gap-1 text-[10px] font-medium text-muted-foreground bg-secondary/50 px-1.5 py-0.5 rounded border border-transparent group-hover:border-border/50 transition-colors">
                        <List class="w-2.5 h-2.5" />
                        {{ item.count }} Problems
                      </span>
                      <div class="flex items-center gap-1 text-[10px] font-bold text-primary opacity-0 -translate-x-2 transition-all duration-300 group-hover:opacity-100 group-hover:translate-x-0">
                        Start
        ...
        <CarouselPrevious class="left-0 -translate-x-1/2 bg-background/90 backdrop-blur shadow-md border-muted hover:bg-background hover:border-primary" />
        <CarouselNext class="right-0 translate-x-1/2 bg-background/90 backdrop-blur shadow-md border-muted hover:bg-background hover:border-primary" />
      </Carousel>
    </section>
