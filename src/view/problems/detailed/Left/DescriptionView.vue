  ...
  <section class="space-y-6">
    <section class="space-y-3">
      <header class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div class="space-y-1">
          <h1 class="text-lg font-semibold leading-tight">{{ props.problem.title }}</h1>
          <p class="text-sm text-muted-foreground">Level up your skills with this prompt.</p>
        </div>
        <Badge
          variant="secondary"
      ...
      >
        <span class="font-semibold uppercase tracking-wide text-[11px] text-foreground/70"
          >Companies:</span
        >
        <span
    ...
    <section v-if="props.problem.examples.length" class="space-y-3">
      ...
      >
        <p class="text-xs font-medium text-muted-foreground">Example {{ index + 1 }}</p>
