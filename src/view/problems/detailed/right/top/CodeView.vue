...
import { AlignLeft, Bookmark, RotateCcw, Maximize2, Scan, ChevronDown } from 'lucide-vue-next'
...
const editorLanguage = computed(() => languageMeta.value?.value ?? 'typescript')
const editorTheme = computed(() => (prefersDark.value ? 'vs-dark' : 'vs-light'))
const activeLanguageLabel = computed(() => languageMeta.value?.label ?? 'Select language')
          ...
          <MenubarContent class="min-w-[200px] p-1">
            ...
            >
              <span class="flex items-center gap-2">
                <CheckIcon
                  class="h-3.5 w-3.5"
                  :class="language.value === activeLanguageValue ? 'opacity-100' : 'opacity-0'"
                />
                {{ language.label }}
              </span>
            </MenubarItem>
