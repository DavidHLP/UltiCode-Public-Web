...
import type { ProblemTestCase, ProblemTestCaseInput } from '@/mocks/schema/problem-detail'
...
const createEmptyInputs = (template?: ProblemTestCase): ProblemTestCaseInput[] => {
...
const activeCase = computed(() => {
  ...
  return localCases.value.find((t) => t.id === activeId.value) ?? localCases.value[0]
})
  ...
  <div class="flex h-full flex-col gap-4">
    ...
    <div v-if="activeCase" class="space-y-4 text-xs md:text-sm">
      <div class="space-y-2">
        ...
        <p v-else class="text-muted-foreground">No predefined inputs for this case.</p>
      </div>
    </div>
  </div>
</template>
