...
const statusCategories: Array<'Solved' | 'Attempted' | 'Todo'> = ['Solved', 'Attempted', 'Todo']
...
const difficultyInsights = computed(() => {
  return difficultyBuckets.map((difficulty) => {
    const filtered = problems.value.filter((problem) => problem.difficulty === difficulty)
    const solved = filtered.filter((problem) => problem.status === 'solved').length
    const attempted = filtered.filter((problem) => problem.status === 'attempted').length
      ...
      <div class="space-y-4">
        ...
        <div class="flex items-end justify-between">
          <div class="flex items-end gap-2">
            <h1 class="text-2xl font-bold">{{ currentList?.name || 'Problem List' }}</h1>
          </div>
        ...
        <!-- 描述 -->
        <p v-if="currentList?.description" class="text-sm text-muted-foreground">
        ...
        <!-- 图表区域 -->
        <div v-if="problems.length" class="grid grid-cols-1 gap-6 xl:grid-cols-3">
    ...
    <template v-if="problems.length === 0">
      <div class="flex h-64 items-center justify-center">
        <div class="text-center">
          <p class="text-lg font-medium text-muted-foreground">No problems in this list</p>
          <p class="mt-2 text-sm text-muted-foreground">Add problems to get started</p>
