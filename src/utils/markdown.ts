...
const groupFencesPlugin = (md: MarkdownIt) => {
  md.core.ruler.push("group_fences", (state) => {
    ...
    while (i < tokens.length) {
      ...
      if (token.type === "fence") {
        ...
        while (j < tokens.length && tokens[j].type === "fence") {
          group.push(tokens[j]);
          j++;
        }
  ...
  md.renderer.rules.code_group = (tokens, idx, options, env, self) => {
    ...
    group.forEach((fence: any, index: number) => {
    ...
    group.forEach((fence: any, index: number) => {
      ...
      if (options.highlight) {
        highlightedCode = options.highlight(fence.content, langName, "") || md.utils.escapeHtml(fence.content);
      } else {
